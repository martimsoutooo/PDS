...
public class WSSolver {
    ...
    static ArrayList<String> wordsToSearch(String line, ArrayList<String> words) {
        ...
        for (String newWord : wordsInLine) {
            if (Functions.verifyWord(newWord)) {
                String upperNewWord = newWord.toUpperCase();
                boolean addWord = true;
    
               
                for (int i = 0; i < words.size(); i++) {
                    String existingWord = words.get(i);
                   
                    if (upperNewWord.contains(existingWord)) {
                        words.set(i, upperNewWord);
                        addWord = false;
                        break;
                    }
                   
                    if (existingWord.contains(upperNewWord)) {
                        addWord = false;
                        break;
                    }
                }
